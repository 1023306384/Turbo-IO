...
面向开发者的雷鸟 iO 智能眼镜 SDK、手机 App 扩展与眼镜端应用研究项目，覆盖 **iOS、Android、原生鸿蒙 HarmonyOS**。从自有 AI、录音和 Agent 接入，进一步探索 **Turbo Display 显示测试工具**与**眼镜端原生导航应用**。不是给小白直接安装的成品 App。
...
[新增显示与导航](#native-apps) · [功能对比](#各端功能对比) · [AI 帮你接入](#让-claude-code--codex-帮你接入) · [V1 快速开始](#快速开始) · [Roadmap](#roadmap)
...
整理于 **2026-09-22**；各端沿用各自已记录的验收结果，并非当天全量复测。下表比较已公开的四条 SDK/App 路线，**TDP1/TNV1 已单独发布，但不是各端默认能力；当前配套手机实现为可选 iOS 实验构建**。
...
模型 Key、搜索 Key、个人提示词与私有知识库由使用者自己配置。二进制例外仅限上方原厂砸壳 IPA，以及独立标注警告的 R3、TNV1 实验固件/原厂内容基线 Release；不提供合并后手机成品包或个人签名材料。固件中的原厂代码/资源版权不变，实验附件不是官方升级推荐；未知二进制版本不得跳过兼容检查。iOS 详细版本/UUID见 [V2文档](official-addon/README.md#1-兼容性门槛)，Android输入校验见 [Android文档](android-addon/README.md#2-兼容与安装边界)。
...
眼镜端应用路线则只修改 AP：Turbo Display 接收像素，TNV1 接收导航数据并在眼镜侧绘制，以不同方式扩展显示能力；未修改其余13个固件负载，但官方完整OTA仍可能写入未变化的负载。
