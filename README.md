...
## 新增：高德步行导航 → 雷鸟 iO 眼镜文字指引

V2 增加手机地图搜索、单击选起终点、缩放、路线规划与步行模拟，并把转向、距离和道路文字持续送到眼镜字幕页面。**不是把手机地图投屏，也不是在眼镜安装高德 App。**

入口：**雷鸟 App → TurboIO → 资料 → 步行导航 · 眼镜字幕**。先配置自己的高德 iOS Key，再搜索/选点 → 规划路线 → 开始模拟 → 开启眼镜字幕显示。正常连接后不再要求每次先手动操作官方实时字幕。

**先看边界：眼镜常亮字幕当前只开放模拟验收，4分钟保护；手机实际步行仅前台，户外、长时间后台和全固件兼容尚未完成验收。** 私用前身已获得免重复采样镜片显示确认；本次搜索/选点交互改版通过本机测试，真机在线搜索仍待补充。它是开发者研究能力，不是可依赖的成品出行导航。
