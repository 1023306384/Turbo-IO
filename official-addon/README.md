...
当前常亮字幕仅开放模拟验收，4分钟保护；实际户外和后台不作为已完成能力。官方1.0.2(67) + StrixOS1.0.3.15的私用前身已确认免重复预览显示；本次官方1.0.4(195) + StrixOS1.0.4.8的升级后导航也收到正常反馈，不能外推全部模式及固件。完整构建及使用流程见 **[导航教程](docs/NAVIGATION.md)**。
...
**未实现/未完整验收：** 日常扩展中的任意眼镜 UI、像素截图、TTS 接管、完整官方历史导出、新闻重启免校准、所有固件兼容、知识库手机→Mac→镜片闭环、APNs 主动推送、网页待办全链路双向同步。固件修改另见下文独立的高风险 R3 实验，不是日常扩展能力。非越狱 Air 的私用前身已有实际功能反馈，不代表本公开源码在所有设备上均已验收。
