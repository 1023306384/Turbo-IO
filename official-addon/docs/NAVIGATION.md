...
| 路线 | 手机步行算路、全览、分段详情、模拟/实际步行切换；先规划后开始，改起终点使旧路线失效 |
...
| 实际出行 | 手机实际步行代码已接入但仅前台；**眼镜字幕暂不开放实际步行，不适用于驾车/骑行，更不是安全导航设备** |

不要边开车边测试，不以实验镜片文字替代道路观察或成熟导航产品。地图及搜索依赖网络；本项目不提供离线地图、导航TTS接管、锁屏持续GPS或路线轨迹存储。
...
3. App内进入 **TurboIO → 资料 → 步行导航 · 眼镜字幕 → 右上角更多 → 高德 Key 设置**，粘贴自己的Key。
4. Key只存本机钥匙串，不回显旧值。公开版没有维护者Key或从私用包自动导入Key的逻辑。要换Key时先重启App，在初始化地图前修改。
5. 点击“开启地图”并阅读隐私说明后决定是否同意。拒绝时不初始化地图SDK。主动点“我的位置”或实际步行才请求定位；模拟不必使用真实位置。

## 5. 第一次运行建议用模拟

1. 在雷鸟App首页确认眼镜正常连接，结束录音、提词、字幕、智记和对话。V2在同一宿主内工作，不需要另开V1；不要为使用导航反复解绑。
2. 进入导航。点“去哪里？”输入地点、可选城市，再点 **搜索地点** 或键盘搜索。仅输入不会每字上传查询；搜索中可点取消。
...
6. 点 **规划路线**，核对路线与步行距离/时间，再点 **开始模拟**。右上角更多有公开的北京演示路线，便于不使用私人位置验收。
...
修改已规划但尚未开始的起终点会清掉旧路线，须重新规划。真正导航过程中先停止再更换目的地。字幕4分钟保护并非故障，不用自动重开循环规避保护。
...
| 眼镜显示按钮不可用 | 先开始真实高德模拟，等待新鲜导航回调；停止其他显示任务。不是实际步行模式，也不是离线夹具 |
...
这是业务内容与受限组件协议研究，不是HTML、Canvas、像素截图或替换系统桌面。后续优先研究真实步行安全验收、生命周期、长时间后台和异常恢复，而非先取消保护计时。
...
xcrun simctl launch booted io.turboio.research.preview --navigation --navigation-interaction-check
```
