...
BOOL TMIsScopedCall(NSString *method,NSDictionary *args){return NSThread.isMainThread&&ScopedArgs&&args==ScopedArgs&&[method isEqual:@"rayneonet_sendFile"];}
...
static BOOL Valid(NSData *d){TMPacket p;return [d isKindOfClass:NSData.class]&&tm_decode(d.bytes,d.length,&p);}
@implementation TMTransport {
...
- (void)send:(NSData *)packet task:(NSString *)task submitted:(TMSubmitted)done{
 ...
 NSURL *url=[dir URLByAppendingPathComponent:@"turbo-music.tmu"];
 ...
 TMSubmitted finish=^(BOOL ok,NSString *nativeTask){NSString *pinned=[nativeTask copy];dispatch_async(dispatch_get_main_queue(),^{if(completed)return;completed=YES;done(ok,pinned);});};
