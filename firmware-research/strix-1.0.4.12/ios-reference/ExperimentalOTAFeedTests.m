...
int main(int argc,const char **argv){@autoreleasepool{
    ...
    assert([metadata[@"apkSize"] intValue]==9249414);assert([metadata[@"versionName"] isEqual:@"0100040012"]);assert([metadata[@"versionCode"] intValue]==100040012);
    ...
    NSDictionary *head=Fetch(port,@"HEAD",download,@"");assert([head[@"body"] length]==0);assert([head[@"header"] containsString:@"Content-Length: 9249414"]);
    ...
    part=Fetch(port,@"GET",download,@"Range: bytes=9249400-\r\n");assert([part[@"code"] intValue]==206);assert([part[@"body"] length]==14);
    for(NSString *range in @[@"bytes=-1",@"bytes=2-1",@"bytes=9249414-",@"bytes=1-2,4-5",@"bytes=99999999999999-"]){assert([Fetch(port,@"GET",download,[NSString stringWithFormat:@"Range: %@\r\n",range])[@"code"] intValue]==416);}
    ...
    // Accept/close leaves TIME_WAIT. Rebind must succeed once async stop closes
    // the listener; duplicate-live-listener rejection above must still hold.
    TIOExperimentalOTAFeed *restarted=[TIOExperimentalOTAFeed new];BOOL rebound=NO;
    for(int retry=0;retry<100&&!rebound;retry++){usleep(5000);rebound=[restarted startOnPort:port error:&error];}
    assert(rebound);assert(Query(port,path)[@"data"]==NSNull.null);
    assert(![restarted.status[@"armed"] boolValue]);[restarted stop];
