...
- (BOOL)allows:(NSData *)payload device:(NSString *)device uptime:(NSTimeInterval)now{
    @synchronized(self){
        ...
        NSUInteger n=0;
        if(t==3&&dict&&[j count]==1&&Number(j[@"OtaSize"],&n)&&n==9249414&&!binary.length)return YES;
        if(!_stage){if(t==4)_failure=@"R3 试刷尚未授权；开始指令未发送。请先在实验固件页完成 R3 风险确认。";return NO;}
...
BOOL TIOOTAFlashBuild(void){
    ...
    return [[NSBundle.mainBundle objectForInfoDictionaryKey:@"TIOExperimentalOTAQueryRouting"] isEqual:@"ios105-r3-loopback-flash-gated"];
#else
    return NO;
#endif
}
...
BOOL TIOOTAFlashProtected(void){return TIOOTAFlashBuild()&&[TIOOTAGuardStatus()[@"transportHookReady"] boolValue];}
void TIOOTAFlashDisableAutoUpdateIfRequested(void){
    if(!TIOOTAFlashProtected()||![NSProcessInfo.processInfo.environment[@"TIO_OTA_DISABLE_AUTO_ONCE"] isEqual:@"DISABLE_FOR_PRIVATE_R3"])return;
...
BOOL TIOOTAFlashBlockCall(id call){
    ...
    NSString *lower=method.lowercaseString;return [lower containsString:@"sendfile"]||[lower containsString:@"ota"]||[lower containsString:@"upgrade"];
}
