...
int main(int argc,const char **argv){@autoreleasepool{
    assert(argc==2);NSURL *dir=[NSURL fileURLWithPath:[NSString stringWithUTF8String:argv[1]]];
    ...
    assert([g allows:F(1,nil,nil) device:@"TEST_DEVICE" uptime:10]);assert([g allows:F(2,nil,nil) device:@"TEST_DEVICE" uptime:10]);assert([g allows:F(3,@{@"OtaSize":@9249414},nil) device:@"TEST_DEVICE" uptime:10]);
    ...
    for(NSDictionary *badRange in @[@{@"Name":@"../nuttx_ap.bin",@"Start":@0,@"Size":@1},@{@"Name":@"nuttx_ap.bin",@"Start":@(-1),@"Size":@1},@{@"Name":@"nuttx_ap.bin",@"Start":@0,@"Size":@YES},@{@"Name":@"nuttx_ap.bin",@"Start":@9473375,@"Size":@2}]){
    ...
    NSLog(@"PASS: closed by default, 900s grant, device binding, Mode 2 only, version/manifest ordering, all %lu slices (%lu bytes) match frozen R3, old AP MD5 and changed chunks rejected; no device IO",(unsigned long)slices,(unsigned long)total);
}return 0;}
