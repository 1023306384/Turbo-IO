...
NSString *TIOExperimentalOTASHA(void){return @"307a0d41aa76b3ed91a8fcc07b09329d76d78a51584c9954c7896d64b1ad9a81";}
...
NSDictionary *TIOCheckExperimentalOTA(NSData *data,NSError **error){
    if(data.length!=9468398)return Fail(error,@"文件大小不符。只接受本次 TMU1 实验 ZIP，未保存、未发送。");
    ...
    return @{@"schema":@1,@"candidate":@"Turbo Music TMU1",@"baseFirmware":@"1.0.4.12",
...
static NSData *Read(NSURL *source,NSError **error){
    ...
    while((n=[stream read:buffer maxLength:sizeof(buffer)])>0){
        if(data.length+(NSUInteger)n>9468398){[stream close];return Fail(error,@"文件过大；未保存、未发送。");}
...
static NSDictionary *PayloadPins(void){return @{
    @"OtaFileInfo.json":@[@2515,@"03767e475ceefd4c3e779d58e85fb2f2b7893485741c8a20197c3754cc855fe7"],
    ...
    @"nuttx_ap.bin":@[@9902600,@"15da0e4ed255587f3b0f2dc422aff9c01d4ea48f3cf71c73407f3fcd6b2c936f"],
...
NSDictionary *TIOCheckExperimentalOTADirectory(NSURL *directory,NSError **error){
    ...
    NSMutableDictionary *identities=[NSMutableDictionary new];NSString *problem=@"文件集合与 TMU1 不符，可能是旧缓存或残缺目录；未发送。";
    for(NSString *name in [pins.allKeys sortedArrayUsingSelector:@selector(compare:)]){
        ...
        close(fd);if(!ok)problem=[NSString stringWithFormat:@"%@ 与固定 TMU1 不一致或读取期间改变；未发送。",name];
    }
    ...
    return @{@"candidate":@"Turbo Music TMU1",@"checkedMembers":@15,@"integrityPassed":@YES,@"snapshotOnly":@YES,@"transferSessionBound":@NO,@"sent":@NO,@"flashAuthorized":@NO};
}
