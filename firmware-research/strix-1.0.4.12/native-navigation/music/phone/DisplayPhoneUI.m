...
BOOL TDPPhonePauseForOTA(void){
 if(!NSThread.isMainThread||Session.busy||NavFeed.active||!TNVPauseForOTA()||!TMMusicPauseForOTA())return NO;
...
BOOL TDPPhoneRouteReply(NSDictionary *event,void(^completion)(BOOL)){
 ...
 NSData *data=m[@"payload"];TDPReply r;if(!TMMusicReply(event,NULL)&&!TNVDecodeReply(event,NULL)&&!tdp_carrier_decode(15,data.bytes,data.length,&r))return NO;
...
static BOOL Setup(void){
 if(!TNVPauseForOTA()||!TMMusicPauseForOTA())return NO;
