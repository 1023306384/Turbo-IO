...
int main(void){TNWidgets api={NULL,ready,make,make,label,place,buffer,text,show,destroy};
 ...
 assert(!strcmp(((Node *)v.lines[0])->text,"four"));
 assert(wr_back(r));uint32_t request=r->event_id;assert(!wr_back(r));
 assert(wr_view_draw(&v,r));assert(strstr(((Node *)v.lines[1])->text,"返回"));
 staged(r,2);assert(wr_view_draw(&v,r));assert(r->returning&&!r->pending&&!r->valid&&r->event_id==request&&r->event==WR_SHELF);
 staged(r,1);assert(wr_view_draw(&v,r));assert(!r->returning&&r->valid);
 assert(wr_view_close(&v));assert(!v.root&&deleted==1);free(r);
