...
typedef struct {WRReader state;WRView view;WRSlot *slot;void *app,*timer;uint32_t token,last_wheel,last_emit;unsigned emitted;bool closing,retired,focus;bool requested,opening,transitioned;uint32_t open_started;unsigned open_reason;WRInput input;uint32_t input_revision,last_back;bool backed;} Control;
