...
if 'focus_screen_on' in symbols:mocks[symbols['focus_screen_on']&~1]='focus_screen_on'
def hook(_u,addr,size,data):
 ...
 elif n=='stream_timer_create':v=timernext;timernext+=64;timers[v]=(r(0),r(2));put(v+8,r(0));put(v+12,r(2));ret(v)
 elif n=='stream_timer_next':
  keys=list(timers);index=keys.index(r(0))+1 if r(0) in keys else 0;ret(keys[index] if index<len(keys) else 0)
 elif n=='focus_screen_on':ret(True)
