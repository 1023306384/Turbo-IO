...
before=struct.unpack_from('<I',replies[-1],20)[0]
for _ in range(50):call('tm_slot_wheel',music,1);tick(10)
assert struct.unpack_from('<I',replies[-1],20)[0]==before
for _ in range(3):call('tm_slot_wheel',music,200);tick(20)
tick(1100);assert replies[-1][5]==4
assert all(objects[k]['xy'][0]>=16 for k in labels if k not in rows),'left text safe inset'
