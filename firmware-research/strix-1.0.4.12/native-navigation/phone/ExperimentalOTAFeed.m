...
- (BOOL)startOnPort:(uint16_t)port error:(NSError **)error{
    @synchronized(self){
        ...
        // Allow reuse after a stopped process leaves TIME_WAIT connections.
        // Never SO_REUSEPORT: a live listener must still reject a second server.
        int reuse=1;
        if(setsockopt(fd,SOL_SOCKET,SO_REUSEADDR,&reuse,sizeof(reuse))||bind(fd,(struct sockaddr *)&addr,sizeof(addr))||listen(fd,4)||fcntl(fd,F_SETFL,O_NONBLOCK)<0){int code=errno;close(fd);if(error)*error=FeedError([NSString stringWithFormat:@"绑定本机端口失败 errno=%d；未改用其他地址或远程服务。",code]);return NO;}
...
- (void)handleClient:(int)fd{
    ...
    @synchronized(self){[self expireLocked];
        if(!valid||![headers[@"host"] isEqual:[NSString stringWithFormat:@"127.0.0.1:%u",_port]]||headers[@"transfer-encoding"]||(headers[@"content-length"]&&![headers[@"content-length"] isEqual:@"0"]))code=400;
            ...
            if(_archive&&[kind isEqual:@"firmware"])data=@{@"taskName":@"Turbo Navigation TNV1 RESEARCH",@"appName":@"Strix OS",@"packageName":@"Strix OS",@"versionName":@"0100040012",@"versionCode":@100040012,@"upgradeType":@1,@"userScope":@0,@"updateDesc":@"HIGH RISK RESEARCH: AP modified; other 13 payloads unchanged. Limited user acceptance only; no recovery guarantee.",@"appIcon":@"",@"apkSize":@(_archive.length),@"apkMd5":_md5,@"downloadUrl":[NSString stringWithFormat:@"http://127.0.0.1:%u/p/%@/Strix_OS_1.0.4.12.zip",_port,_token]};
...
BOOL TIOBeginExperimentalOTAPreparation(NSError **error){
    ...
    if(!SharedFeed){if(error)*error=FeedError(BootstrapFailure?:@"本机服务不可用，请停止检查更新并核对当前进程报告。");TIORefreshExperimentalOTAReport();return NO;}
    BOOL ok=[SharedFeed armArchive:archive lifetime:900 error:error];TIORefreshExperimentalOTAReport();return ok;
}
...
void TIORefreshExperimentalOTAReport(void){
#if TARGET_OS_IPHONE
    if(!SharedFeed&&!BootstrapFailure)return;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,NSEC_PER_SEC),dispatch_get_global_queue(QOS_CLASS_UTILITY,0),^{@autoreleasepool{
        ...
        if([fm createDirectoryAtURL:directory withIntermediateDirectories:YES attributes:@{NSFilePosixPermissions:@0700} error:&error]&&[directory setResourceValue:@YES forKey:NSURLIsExcludedFromBackupKey error:&error]){
            ...
            report[@"processIdentifier"]=@(NSProcessInfo.processInfo.processIdentifier);
            if(TIOOTAFlashBuild()){report[@"build"]=[NSBundle.mainBundle objectForInfoDictionaryKey:@"TIOExperimentalOTABuild"]?:@"TNV1-SOURCE-01";report[@"flashGate"]=TIOOTAFlashStatus();report[@"flashAuthorized"]=report[@"flashGate"][@"authorized"];}
...
void TIOStartExperimentalOTAFeedIfMarked(void){
    ...
    dispatch_once(&once,^{
        ...
        if(![[bundle objectForInfoDictionaryKey:@"TIOExperimentalOTAQueryRouting"] isEqual:@"ios105-tnv1-loopback-disabled"]&&!TIOOTAPreparationBuild()&&!TIOOTAFlashBuild())return;
