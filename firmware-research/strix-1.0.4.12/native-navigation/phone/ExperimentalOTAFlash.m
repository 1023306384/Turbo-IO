...
- (BOOL)allows:(NSData *)payload device:(NSString *)device uptime:(NSTimeInterval)now{
    @synchronized(self){
        ...
        NSUInteger n=0;
        if(t==3&&dict&&[j count]==1&&Number(j[@"OtaSize"],&n)&&n==9258094&&!binary.length)return YES;
        if(!_stage){if(t==4)_failure=@"TNV1 试刷尚未授权；开始指令未发送。请先在实验固件页完成 TNV1 风险确认。";return NO;}
...
BOOL TIOOTAFlashBuild(void){
    ...
    return YES;
#else
    return [[NSBundle.mainBundle objectForInfoDictionaryKey:@"TIOExperimentalOTAQueryRouting"] isEqual:@"ios105-tnv1-loopback-flash-gated"];
#endif
#else
    return NO;
#endif
...
#if TARGET_OS_IPHONE && TIO_DISPLAY_FLASH
    if(![[NSBundle.mainBundle objectForInfoDictionaryKey:@"TIOExperimentalOTAQueryRouting"] isEqual:@"ios105-tnv1-loopback-flash-gated"])return NO;
#endif
    return TIOOTAFlashBuild()&&[TIOOTAGuardStatus()[@"transportHookReady"] boolValue];
}
void TIOOTAFlashDisableAutoUpdateIfRequested(void){
    if(!TIOOTAFlashProtected()||![NSProcessInfo.processInfo.environment[@"TIO_OTA_DISABLE_AUTO_ONCE"] isEqual:@"DISABLE_FOR_PRIVATE_TNV1"])return;
...
#if TIO_NATIVE_NAV
    scopedImage=scopedImage||TNVIsScopedCall(method,args);
#endif
#endif
#endif
