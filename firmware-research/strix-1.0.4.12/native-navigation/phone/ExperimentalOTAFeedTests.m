...
int main(int argc,const char **argv){@autoreleasepool{
    ...
    assert([metadata[@"apkSize"] intValue]==9258094);assert([metadata[@"versionName"] isEqual:@"0100040012"]);assert([metadata[@"versionCode"] intValue]==100040012);
    ...
    assert([metadata[@"apkMd5"] isEqual:md5]);assert(![metadata[@"apkMd5"] isEqual:@"dae276f1663c6a5808c1a75ea2151f4e"]);
    ...
    NSDictionary *head=Fetch(port,@"HEAD",download,@"");assert([head[@"body"] length]==0);assert([head[@"header"] containsString:@"Content-Length: 9258094"]);
    ...
    part=Fetch(port,@"GET",download,@"Range: bytes=9258080-\r\n");assert([part[@"code"] intValue]==206);assert([part[@"body"] length]==14);
    for(NSString *range in @[@"bytes=-1",@"bytes=2-1",@"bytes=9258094-",@"bytes=1-2,4-5",@"bytes=99999999999999-"]){assert([Fetch(port,@"GET",download,[NSString stringWithFormat:@"Range: %@\r\n",range])[@"code"] intValue]==416);}
