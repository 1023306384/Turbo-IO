...
int main(int argc,const char **argv){@autoreleasepool{
    ...
    for(NSDictionary *row in manifest)if([row[@"Name"] isEqual:@"nuttx_ap.bin"]){assert([row[@"Md5"] isEqual:@"dae276f1663c6a5808c1a75ea2151f4e"]);assert([row[@"Size"] unsignedIntegerValue]==9487724);}
    ...
    assert([g allows:F(1,nil,nil) device:@"TEST_DEVICE" uptime:10]);assert([g allows:F(2,nil,nil) device:@"TEST_DEVICE" uptime:10]);assert([g allows:F(3,@{@"OtaSize":@9258094},nil) device:@"TEST_DEVICE" uptime:10]);
    ...
    assert(slices==366&&total==18385897);
    ...
    for(NSDictionary *badRange in @[@{@"Name":@"../nuttx_ap.bin",@"Start":@0,@"Size":@1},@{@"Name":@"nuttx_ap.bin",@"Start":@(-1),@"Size":@1},@{@"Name":@"nuttx_ap.bin",@"Start":@0,@"Size":@YES},@{@"Name":@"nuttx_ap.bin",@"Start":@9487723,@"Size":@2}]){
    ...
    NSLog(@"PASS: closed by default, 900s grant, device binding, Mode 2 only, version/manifest ordering, all %lu slices (%lu bytes) match frozen TNV1, old AP MD5 and changed chunks rejected; no device IO",(unsigned long)slices,(unsigned long)total);
}return 0;}
