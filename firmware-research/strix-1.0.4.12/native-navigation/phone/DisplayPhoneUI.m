...
BOOL TDPPhonePauseForOTA(void){
 if(!NSThread.isMainThread||Session.busy||NavFeed.active||!TNVPauseForOTA())return NO;
...
BOOL TDPPhoneRouteReply(NSDictionary *event,void(^completion)(BOOL)){
 ...
 NSData *data=m[@"payload"];TDPReply r;if(!TNVDecodeReply(event,NULL)&&!tdp_carrier_decode(15,data.bytes,data.length,&r))return NO;
