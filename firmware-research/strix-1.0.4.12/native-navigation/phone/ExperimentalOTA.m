...
NSString *TIOExperimentalOTASHA(void){return @"e2a76fdcf0d3d07d766a7329d2d93b32350cb8309b73fc9c73726498fafddecb";}
...
NSDictionary *TIOCheckExperimentalOTA(NSData *data,NSError **error){
    if(data.length!=9258094)return Fail(error,@"文件大小不符。只接受本次 TNV1 实验 ZIP，未保存、未发送。");
    ...
    return @{@"schema":@1,@"candidate":@"Turbo Navigation TNV1",@"baseFirmware":@"1.0.4.12",
...
static NSData *Read(NSURL *source,NSError **error){
    ...
    while((n=[stream read:buffer maxLength:sizeof(buffer)])>0){
        if(data.length+(NSUInteger)n>9258094){[stream close];return Fail(error,@"文件过大；未保存、未发送。");}
...
static NSDictionary *PayloadPins(void){return @{
    @"OtaFileInfo.json":@[@2515,@"fcc2020d6282c0db21b790d4291e506b3a6e6fcb8a3bbf8ca73d412ca3541df2"],
    ...
    @"nuttx_ap.bin":@[@9487724,@"1a0e670cdcdd59d4c987eb72767f37c12e6313e525ddaeab8bed18ae3278d867"],
...
NSDictionary *TIOCheckExperimentalOTADirectory(NSURL *directory,NSError **error){
    ...
    NSMutableDictionary *identities=[NSMutableDictionary new];NSString *problem=@"文件集合与 TNV1 不符，可能是旧缓存或残缺目录；未发送。";
    for(NSString *name in [pins.allKeys sortedArrayUsingSelector:@selector(compare:)]){
        ...
        close(fd);if(!ok)problem=[NSString stringWithFormat:@"%@ 与固定 TNV1 不一致或读取期间改变；未发送。",name];
    }
    ...
    return @{@"candidate":@"Turbo Navigation TNV1",@"checkedMembers":@15,@"integrityPassed":@YES,@"snapshotOnly":@YES,@"transferSessionBound":@NO,@"sent":@NO,@"flashAuthorized":@NO};
}
