...
p=argparse.ArgumentParser();p.add_argument('candidate',type=Path);p.add_argument('--report',default='menu10-native-arm.json');a=p.parse_args()
d=a.candidate;report=json.loads((d/'report.json').read_text())
assert report['menuProfile']=='native-ten-TDP1-TNV1-TMU1'
...
def scenario(fail_at=None):
 ...
 nextobj=[ROOT+512];creations=[0];direction=[1];wake=[False];key=[0x3a];now=[1000];opened=[];launched=[];trace=[];nav=[False];music=[False];SLOT=0x201e0000;MUSIC=SLOT+128
 ...
 for name in ['tm_slot_create','tm_slot_destroy','tm_slot_hidden','tm_slot_visible','tm_slot_open','tm_slot_wheel']:calls[syms[name]&~1]=name
 ...
 def hook(uc,address,size,data):
  ...
  if not name:return
  if name=='tn_slot_create':u.mem_write(SLOT,bytes(28));ret(SLOT)
  elif name=='tm_slot_create':u.mem_write(MUSIC,bytes(24));ret(MUSIC)
  elif name=='tm_slot_visible':ret(music[0])
  elif name=='tm_slot_open':music[0]=fail_at!='musicpage';ret(music[0])
  elif name in ['tm_slot_destroy','tm_slot_hidden']:music[0]=False;ret()
  elif name=='tm_slot_wheel':assert music[0];ret()
 ...
 for n in range(-40,310):
  assert call(0x1079911a,n,0x123,0x456,0x789)==15+30*max(0,min(9,n))
  assert [u.reg_read(r) for r in REG[1:]]==[0x123,0x456,0x789],'Leaf helper clobbers native caller registers'
  assert call(0x10799130,n,0x123,0x456,0x789)==max(0,min(299,n))//30
  assert [u.reg_read(r) for r in REG[1:]]==[0x123,0x456,0x789]
 if fail_at is None:
  assert len([o for o in objects.values() if o['parent']==word(APP+0x60)])==10
  assert word(APP+0xdc) and word(APP+0xe4),'Missing eighth row/icon'
 for frame in list(range(300))+list(range(299,-1,-1)):
  ...
  if fail_at is None:
   ...
   if frame==285:
    assert objects[word(MUSIC)]['opa']==255 and not objects[word(MUSIC)]['hidden']
    assert objects[word(MUSIC+8)]['opa']==255 and not objects[word(MUSIC+8)]['hidden']
    assert objects[word(SLOT+8)]['hidden'] and objects[icon]['hidden']
    assert objects[word(MUSIC+12)]['size']==(12,4)
 ...
 selected(8);call(0x1079a310,APP,600);assert word(APP+0x88)==9
 for delta in [-1,0,1,0,-1,1]*20:call(0x1079a310,APP,delta);assert word(APP+0x88)==9
 selected(9);call(0x1079a310,APP,-600);assert word(APP+0x88)==8
 for start in range(10):
  for delta in [-1000000,-600,-2,2,600,1000000]:
   selected(start);call(0x1079a310,APP,delta);assert word(APP+0x88)<=9
 fp(APP+0xa8,99);call(0x107992f4,APP);assert 9.46<readfp(APP+0xa8)<9.47
 ...
 fp(APP+0xa8,9);write(APP+0x90,0);call(0x10799d6e,APP);assert word(APP+0x90)==285
 ...
 call(0x1079a784,APP);selected(9);call(0x1079a890,APP,0);assert music[0]==(fail_at!='musicpage');assert len(launched)==7
 if music[0]:call(0x1079a310,APP,-600);assert word(APP+0x88)==9
 call(0x1079a7b8,APP);assert not music[0]
 ...
 return {'failAt':fail_at,'passed':True,'nativeWheelIndex7':True,'nativeReverseDirection':True,
  ...
  'frameSweep':600,'nativeWheelIndex8':True,'nativeWheelIndex9':True,'tenthMenuDistinct':True,'ninthMenuDistinct':True,'originalArrayPointersPreserved':True,'tailCanary':True,'destroyNoDoubleFree':True}
result={'kind':'Native ten-index ARM offline test','apSHA256':hashlib.sha256(ap).hexdigest(),
 'realWheelMath':True,'nativeUIAndPageAreMocks':True,'deviceIO':False,'displayValidated':False,
 'scenarios':[scenario()]+[scenario(n) for n in range(1,41)]+[scenario('page'),scenario('navpage'),scenario('musicpage')]}
